...
function [score,tp,fp, allthreshes] = AUC_Borji_for_combined_ROC(saliencyMap, fixationMap, Nsplits, stepSize)
...
for s = 1:Nsplits
    ...
    %%%allthreshes = fliplr([0:stepSize:double(max([Sth;curfix]))]); %for
    %this experiment, we put all threshes 0.9 through 0
    allthreshes = [0.9:-0.1:0];
